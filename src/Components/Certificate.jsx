...
const Certificate = () => {
  return (
    <div className="p-10 rounded-lg flex flex-col gap-10" style={{ background: "var(--surface-elevated)" }}>
      <h1 className="text-3xl font-bold" style={{ color: "var(--color-text)" }}>Techincal Skills</h1>

      <div className="w-full flex flex-wrap gap-4">
        {skills.map((curr) => (
          <div key={curr.skill} className="w-[7rem] text-center h-[7rem] rounded-full flex flex-col">
            <img src={curr.png} className="w-full p-5" />
            <h1 className="font-bold" style={{ color: "var(--color-text)" }}>{curr.skill}</h1>
          </div>
        ))}
      </div>

      <h1 className="text-3xl font-bold" style={{ color: "var(--color-text)" }}>Other Skills</h1>

      <div className="w-full flex flex-wrap gap-4">
        {otherSkills.map((curr) => (
          <div key={curr.skill} className="w-[7rem] text-center h-[7rem] rounded-full flex flex-col">
            <img src={curr.png} className="w-full p-5" />
            <h1 className="font-bold" style={{ color: "var(--color-text)" }}>{curr.skill}</h1>
          </div>
